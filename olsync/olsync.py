...
def main(ctx, local, remote, project_name, cookie_path, sync_path, olignore_path, delete_choice, keep_old,
    ...
    if ctx.invoked_subcommand is None:
        ...
        project_infos = execute_action(
            ...
            "Querying project details", "Project details queried successfully.",
        ...
        if remote or sync:
            sync_func(
                ...
                create_file_at_from=lambda name: overleaf_client.upload_file(
                    project["id"], project_infos, name, os.path.getsize(name),
                    open(name, 'rb')),
                from_exists_in_to=lambda name: os.path.isfile(name),
                from_equal_to_to=lambda name: open(name, 'rb').read(
                ) == zip_file.read(name),
                from_newer_than_to=lambda name: dateutil.parser.isoparse(project[
                    "lastUpdated"]).timestamp() > os.path.getmtime(name),
                from_name="remote",
                to_name="local",
                verbose=verbose)
        if local or sync:
...
def list_projects(cookie_path, verbose):
    ...
    def query_projects():
        for index, p in enumerate(
                sorted(overleaf_client.all_projects(),
